(ns discuss.handler
  (:require [compojure.core :refer [defroutes routes wrap-routes]]
            [discuss.routes.home :refer [home-routes]]
            
            [discuss.middleware :as middleware]
            [discuss.session :as session]
...
(defn init
  ...
  (timbre/set-config!
    ...
    {:path "discuss.log" :max-size (* 512 1024) :backlog 10})

  (if (env :dev) (parser/cache-off!))
  (start-nrepl)
  ;;start the expired session cleanup job
  (session/start-cleanup-job!)
  (timbre/info "\n-=[ discuss started successfully"
...
(defn destroy
  ...
  []
  (timbre/info "discuss is shutting down...")
  (stop-nrepl)
